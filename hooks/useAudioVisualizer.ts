...
export function useAudioVisualizer(canvasRef: RefObject<HTMLCanvasElement>, active: boolean) {
  ...
  useEffect(() => {
    ...
    if (!active || !canvasRef.current || !navigator.mediaDevices?.getUserMedia) {
      stop();
      return stop;
    }
    ...
    const render = (analyser: AnalyserNode) => {
      ...
      const draw = () => {
        ...
        for (let x = 0; x < width; x += 2) {
          const sample = data[Math.floor((x / width) * data.length)] / 128 - 1;
          const distance = Math.max(0, Math.abs(sample) * height * 1.35);
          const y = height / 2 + Math.sin(x * 0.025) * distance * 0.08 + sample * distance;
